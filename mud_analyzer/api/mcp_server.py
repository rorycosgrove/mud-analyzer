...
# Try to import mcp, with graceful fallback
try:
...
class MudAnalyzerMCPServer:
    def __init__(self, config: Config):
        if not HAS_MCP:
            raise ImportError(f"MCP server requires 'mcp' package: {IMPORT_ERROR}")
...
async def main():
    ...
    if not HAS_MCP:
        print("\n" + "="*60)
        print("🏰 MUD ANALYZER MCP SERVER")
        print("="*60)
        print(f"\n❌ MCP module not available: {IMPORT_ERROR}")
        print("\n📦 Install MCP with:")
        print("   pip install mcp")
        print("\nThe MCP server requires the 'mcp' package to function.")
        print("="*60 + "\n")
        return
...
if __name__ == "__main__":
    ...
    print("\n✅ MCP Server initialization starting...")
